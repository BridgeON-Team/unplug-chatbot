...
@RestController
@RequestMapping("/chatbot/threads")
@RequiredArgsConstructor
public class ChatThreadController {

    private final ChatThreadService chatThreadService;

    @Operation(summary = "채팅 스레드 생성", description = "새로운 채팅 스레드를 생성. 사용자명은 Gateway에서 전달된 X-Auth-Username 헤더로 식별.")
    @PostMapping
    ...
    // 소유자 검증 추가
    @Operation(summary = "단일 스레드 조회", description = "스레드 ID를 기준으로 자신의 채팅 스레드를 조회. 다른 사용자의 스레드 접근 시 SecurityException이 발생.")
    @GetMapping("/{threadId}")
    ...
    @Operation(summary = "내 스레드 목록 조회", description = "로그인된 사용자의 모든 채팅 스레드 목록을 반환.")
    @GetMapping("/me")
    ...
    @Operation(summary = "스레드 삭제", description = "스레드 ID를 기준으로 자신의 스레드를 삭제. 다른 사용자의 스레드는 삭제불가.")
